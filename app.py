...
def admin_dashboard():
    ...
    # Total Funds = MANUAL (ikaw maglalagay)
    total_funds = 0  # 👈 palitan mo anytime

    # ===== FUNDS COMPUTATION (ADD THIS) =====
    total_add = db.session.query(
        func.coalesce(func.sum(AdminFund.amount), 0)
    ).filter(AdminFund.type == "add").scalar()
    
    total_subtract = db.session.query(
        func.coalesce(func.sum(AdminFund.amount), 0)
    ).filter(AdminFund.type == "subtract").scalar()

    total_funds = total_add - total_subtract
    
    member_earnings = db.session.query(
        func.coalesce(func.sum(User.cash_balance), 0)
    ).scalar()
